...
This module owns only the derivative Chroma description index. Durable episode
bodies, metadata, authorization, and exact record loading remain responsibilities
of their Memory stores. Optional Skill metadata keeps the stable Skill ID next
to the existing Record ID without changing ordinary Episodic Memory behavior.
"""
...
class McpEpisodicDescriptionVectorStore:
    ...
    def upsert_description(
        ...
        *,
        ...
        """Embed Description only and persist it under stable Record ID."""
        owner = self._require_text(owner_sub, "owner_sub")
        memory_file_id = self._require_text(file_id, "file_id")
        memory_record_id = self._require_text(
            record_id,
            "record_id",
        )
        ...
        if skill_id is not None:
            metadata["skill_id"] = self._require_text(
                skill_id,
                "skill_id",
            )
        if skill_status is not None:
            metadata["skill_status"] = self._require_text(
                skill_status,
                "skill_status",
            ).upper()
    ...
    def search_descriptions(
        ...
        *,
        ...
        """Return cosine-ranked description hits from allowed records."""
        owner = self._require_text(owner_sub, "owner_sub")
        ...
        allowed_ids = self._clean_record_ids(
    ...
    def delete_records(
        ...
        """Delete vectors by Record ID; an empty request is a no-op."""
        clean_ids = self._clean_record_ids(
            list(record_ids)
        )
        if not clean_ids:
            return
    ...
    def _normalize_query_result(
        ...
        ids = (
            ...
            ._first_result_list(result.get("ids"))
        )
        documents = (
            McpEpisodicDescriptionVectorStore
            ._first_result_list(result.get("documents"))
        )
        metadatas = (
            McpEpisodicDescriptionVectorStore
            ._first_result_list(result.get("metadatas"))
        )
        distances = (
            McpEpisodicDescriptionVectorStore
            ._first_result_list(result.get("distances"))
        )
        ...
        for index, vector_id in enumerate(ids):
            ...
            if metadata.get("skill_status"):
                hit["skill_status"] = str(
                    metadata["skill_status"]
                )
